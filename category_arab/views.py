...
from blog_arab.models import Blog, Category
...
#-----------------------------------------------------------------------
    ...
    context = {
        'article': Blog.objects.filter(category_ar=get_object_or_404(Category, slug=slug)),#المسئول عن جلب المقالات من التصنيف
        'category': get_object_or_404(Category, slug=slug),#المسئول عن جلب المقالات من التصنيف
        'count_category' : Category.objects.all().annotate(post_count=Count('post_category_ar')),
       # 'all_blogs': page_obj,
        ...
        'cate_top': Blog.objects.filter(category_ar=get_object_or_404(Category, slug=slug))[:1],
        'form': form,
    ...
    return render(request, "pages\catgory_arab_blog.html", context)